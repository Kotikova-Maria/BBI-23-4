    ...
    class Task_8 : Task
    {
        ...
        protected override void ParseText(string text)
        {
            ...
            for (int i = 0; i < words.Length; i++)
            {
                ...
                if (count < 50) { lines[k] += words[i] + ' '; } else { k++; count = words[i].Length; lines[k] += words[i] + ' '; lines[k - 1].Trim(); }
            }
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                for (int j = 0; j < words_in_line.Length; j++)
                {
                    symbols += words_in_line[j].Length;
                }

                int spase = 50 - symbols;
                double space_itog = (spase / (words_in_line.Length - 1));


                string space_to_add = "";

                for (int j = 0; j < space_itog; j++)
                {
                    space_to_add += " ";
                }

                string new_line = "";
                for (int j = 0; j < words_in_line.Length - 1; j++)
                {

                    new_line += words_in_line[j] + space_to_add;
                }
