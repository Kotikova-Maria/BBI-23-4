...
namespace _8__защита
{
    ...
    class Task_6 : Task
    {
        ...
        protected override void ParseText(string text)
        {
            ...
            int[] slogi = new int[words.Length];//how many syllables is in each word
    ...
    class Task_9 : Task
    {
        ...
        struct Seqeuence
        {
            ...
            private int count;
            public Seqeuence(string sequence)
            {
                this.sequence = sequence;
                this.count = 1;
            }
            public string GetSequence { get { return sequence; } }
        ...
        protected override void ParseText(string text)
        {
            ...
            string[] words = text.Split(new char[] { ' ', '.', ',', ';', ':', '!', '?' }, StringSplitOptions.RemoveEmptyEntries);
            int b = 0;//possible amount of pairs
            for (int i = 0; i < words.Length; i++)//maybe -1
            {
                b += words[i].Length - 1;
            }

            Seqeuence[] seqeunce = new Seqeuence[b];
            int u = 0;//counter for elements in [] seqeunce
            ...
            for (int i = 0; i < text.Length - 1; i++)
            {
                ...
                for (int o = 0; o < seqeunce.Length; o++)
                {
                    if (seqeunce[o].GetSequence == pair)
                    {
                        seqeunce[o].CounterUp();
                    }
                    else
                    {
                        if (seqeunce[o].GetSequence == null)
                        {
                            seqeunce[o] = new Seqeuence(pair);
                            break;
                        }
                    }
                }
            ...
            Console.WriteLine($"первая пара: {seqeunce[3].GetSequence}, {seqeunce[3].GetCount}");

            List<string> topSequences = new List<string>();

            foreach (Seqeuence seq in seqeunce)
            {
                if (seq.GetCount > 8) { topSequences.Add(seq.GetSequence); }
            }
